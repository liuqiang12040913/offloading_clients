...
public class LiveVideoBroadcaster extends Service implements ILiveVideoBroadcaster, CameraHandler.ICameraViewer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private int maxFPS = 60 - 1;
    private int minFPS = 1 + 1;
                        ...
                        public void run() {
                            ...
                            if (frameQueueIncreased > 10) {
                                ...
                                mGLView.queueEvent(new Runnable() {
                                    @Override
                                    public void run() {
                                        int fps = mRenderer.getFrameRate();
                                        if (fps >= minFPS) {
                                            fps -= stepsize;
                                            mRenderer.setFrameRate(fps);
                                        }
                                        else {
                            ...
                            Log.d(TAG, "video frameRate : " + mRenderer.getFrameRate());
