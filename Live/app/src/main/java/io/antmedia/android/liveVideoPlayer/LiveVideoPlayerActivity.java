...
import static io.antmedia.android.MainActivity.SERVERPORT;
import static io.antmedia.android.MainActivity.SERVER_IP;
...
//  private static String SERVER_IP = "192.168.1.206";
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    Thread thread = new Background_FPS_Thread();
    thread.start();

  }

  /* Define the Thread */
  public class Background_FPS_Thread extends Thread {

    public void run(){
      int counts = 0; // init counts
      TextView thetextview = (TextView) findViewById(R.id.FPSView);

      while (true) {
          // if failed, then try it again
          try {
          ...
          // get socket and then alway sending info
          while (true) {
            try {
            ...
            try {
              DecoderCounters current = player.getVideoDecoderCounters(); // get current counts

              FPS = (current.renderedOutputBufferCount - counts);
              Log.i(TAG, "FPS: " + Integer.toString(FPS));
              thetextview.setText("FPS: " + Integer.toString(FPS) + " ");
              counts = current.renderedOutputBufferCount; // assign it current

              // send data to server
              BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
              DataOutputStream dataOS2 = new DataOutputStream(socket.getOutputStream());
              dataOS2.writeInt(FPS);
